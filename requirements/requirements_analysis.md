...
## Actors
- End User: registers, creates profile, selects services, searches forecasts, rates forecasts, downloads analytics, receives warnings.
- Forecast Service (Partner): pushes forecast updates via API or is polled by the system.
- System Scheduler: ingests and stores forecasts periodically.
- Administrator: manages partner integrations and system configuration (optional).

## Functional Requirements
1. User Registration & Profile
   - Users can register and manage a profile containing areas of interest and selected forecasting services.
...
3. Search and View
   - Users can search forecasts by region/area and view current, hourly and daily forecasts.
4. Service Selection
   - When creating/editing profile, users choose which partner forecasting services to include.
5. Ratings
   - Users can rate predictions on scale 1–5 per forecast item; ratings are stored and aggregated per service and region.
6. Aggregation & Analytics
   - System can return: (a) analytics containing forecasts from two or more services, (b) weighted aggregates (e.g., choose forecast from highest-rated service(s)).
7. Download
...
8. Data Ingestion API (Partner Integration)
   - Provide an API endpoint for partners to push/notify forecast updates per region.
   - System also supports scheduled ingestion/polling to refresh forecasts.
9. Alerts/Warnings
   - System sends warning messages for life-threatening weather to users subscribed to affected areas.
10. Persistence
   - Forecasts and ratings are stored for historical analysis and recalculation of aggregated forecasts.
...
## Use Cases (brief)
- Register / Login
- Create / Edit Profile (areas + selected services)
...
- Partner: Push Forecast Update
- System: Scheduled Polling and Storage

## Acceptance Criteria (examples)
- Users can receive forecasts from at least two partner services for any region.
- Downloaded analytics include at least two independent forecasts when requested.
...
- POST /api/v1/forecasts/push
  ...
  - Payload: { serviceId, regionId, timestamp, forecasts: [{type: CURRENT|HOURLY|DAILY, time, values...}] }
  - Response: 200 OK
...
- How to define regions (geo IDs, polygons, admin units)?

...
**Alternative Flows:**
- **A1: Large File**
  - At step 5, if file too large, system fetches the user's email from the Authentication Service (single-user Appwrite Server SDK lookup, `users.get(userId)`, since email is never stored locally) and sends the download link there

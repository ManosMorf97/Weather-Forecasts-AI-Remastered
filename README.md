...
deployable backend services sharing one SQL Server database.
...
SQL Server schema (owned by `WeatherUserActions`' EF Core migrations).
...
Both services connect to the same **Appwrite** project for authentication — the frontend (not in
this repo) talks to Appwrite directly for sign-up/login/token refresh, and the backend's only job
is to verify the bearer token it receives by calling Appwrite's own `account.get()`, rather than
trusting a client-decoded JWT.
...
Each service is self-contained and has its own setup instructions:
...
Both expect the same machine-level environment variables for the shared SQL Server connection and
Appwrite project (`YOUR_SERVER` / `YOUR_USER` / `YOUR_PASSWORD`, `YOUR_APPWRITE_*`) — see either
